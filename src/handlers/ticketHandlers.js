...
async function getPanels(guildId) {
  const { data } = await supabase.from('panels').select('*').eq('guild_id', guildId);
  return data || [];
}

async function getPanelDepartments(panelId) {
  const { data } = await supabase
    .from('panel_departments')
    .select('*, departments(*)')
    .eq('panel_id', panelId)
    .order('position', { ascending: true });
  return data || [];
...
async function getStaffRoles(guildId) {
  const { data } = await supabase.from('staff_roles').select('*').eq('guild_id', guildId);
  return data || [];
}
